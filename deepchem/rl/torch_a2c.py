...
class A2CLossDiscrete(object):
    ...
    def __call__(self, outputs, labels, weights):
        ...
        action = torch.from_numpy(labels[0])
        advantage = torch.unsqueeze(torch.from_numpy(advantage), 1)
        prob = prob + np.finfo(np.float32).eps
        log_prob = torch.log(prob)
        policy_loss = -torch.mean(advantage * torch.sum(action * log_prob, 1))
        value_loss = torch.mean(torch.square(torch.from_numpy(reward) - value))
        entropy = -torch.mean(torch.sum(prob * log_prob, axis=1))
...
class A2CLossContinuous(object):
    ...
    def __call__(self, outputs, labels, weights):
        ...
        log_prob = torch.sum(distrib.log_prob(action), reduce_axes)
        policy_loss = -torch.mean(torch.from_numpy(advantage) * log_prob)
        value_loss = torch.mean(torch.square(torch.from_numpy(reward) - value))
        dummy_loss = torch.mean(mean)  # Dummy loss involving the mean output
