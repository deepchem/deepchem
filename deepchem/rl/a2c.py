...
class A2CLossContinuous(nn.Module):
    ...
    def __init__(self, value_weight: float, entropy_weight: float, mean_index: int,
                 std_index: int, value_index: int):
...
class A2C:
    ...
    def __init__(self,
        ...
        self._optimizer = self._optimizer._create_pytorch_optimizer(
        ...
        self.model = TorchModel(self._policy,
                               loss=self.loss_fn,
                               batch_size=max_rollout_length,
                               model_dir=model_dir,
    ...
    def fit(self,
        ...
        while step_count < total_steps:
            ...
            if (step_count >= total_steps or
                time.time() >= checkpoint_time + checkpoint_interval):
    ...
    def predict(
        ...
        with torch.no_grad():
            ...
            if self.continuous:
                return (results[self._action_mean_index],
                        results[self._action_std_index],
                        results[self._value_index])
            else:
                return (results[self._action_prob_index],
                        results[self._value_index])
    ...
    def select_action(
            ...
            use_saved_states: bool = True,
            save_states: bool = True) -> np.ndarray:
        self._policy.eval()
        with torch.no_grad():
    ...
    def _predict_outputs(
        ...
        inputs = [
            torch.from_numpy(np.expand_dims(s, axis=0)).float().to(self.device)
            for s in state
        ]
    ...
    def _create_rollout(
        ...
        for _ in range(self.max_rollout_length):
            ...
            rnn_states = [
                ...
                if i in self._rnn_final_state_indices
            ]
            action = self._select_action_from_outputs(
                [r.cpu().numpy() for r in results], False)
        ...
        if not self._env.terminated:
            ...
            with torch.no_grad():
                final_value = (self.discount_factor *
                              self._policy(*inputs)[self._value_index].cpu().numpy()[0])
        else:
            final_value = 0.0
    ...
    def _process_rollout(self, states: List[np.ndarray], actions: List[np.ndarray],
        ...
        for j in range(len(rewards) - 1, 0, -1):
            discounted_rewards[j - 1] += (self.discount_factor *
                                         discounted_rewards[j])
            advantages[j - 1] += (self.discount_factor * self.advantage_lambda *
                                  advantages[j])
        ...
        if self.continuous:
            ...
            actions_matrix = torch.zeros((len(actions), n_actions),
        ...
        inputs = [
            torch.tensor(s).float().to(self.device) for s in state_arrays
        ] + [
            torch.tensor(s).unsqueeze(0).float().to(self.device)
            for s in initial_rnn_states
        ]
        ...
        loss = self.loss_fn(outputs, [actions_matrix],
    ...
    def _process_rollout_with_hindsight(
            self, states: List[np.ndarray], actions: List[np.ndarray],
            initial_rnn_states: List[np.ndarray]) -> None:
    ...
    def _create_model_inputs(
            self, state: np.ndarray, rnn_states: List[np.ndarray]
    ) -> List[torch.Tensor]:
        if not self._state_is_list:
            state = [state]
        state_tensors = [
            torch.from_numpy(np.array(s)).float().to(self.device) for s in state
        ]
        rnn_tensors = [
    ...
    def _process_states(self, states: List[np.ndarray]) -> List[np.ndarray]:
