...
class TensorflowMoleculeEncoder(Model):
    ...
    def __init__(self, model_dir=None, verbose=True, charset=None, latent_rep_size=292):
        """
        ...
        """
        super(TensorflowMoleculeEncoder, self).__init__(model_dir=model_dir, verbose=verbose)
        weights_file = os.path.join(model_dir, "model.h5")
        if os.path.isfile(weights_file):
            m = MoleculeVAE()
            self.model = m.load(charset, weights_file, latent_rep_size=latent_rep_size)
        else:
