...
@pytest.mark.dqc
def test_dftxc_eval_cpu():
    inputs = 'deepchem/models/tests/assets/test_dftxcdata.yaml'
    data = DFTYamlLoader()
    model_dir = tempfile.mkdtemp()
    dataset = (data.create_dataset(inputs, device=torch.device("cpu")))
    model = XCModel("lda_x",
                    batch_size=1,
                    log_frequency=1,
                    mode="classification",
    ...
    predict_dataset = data.create_dataset(inputs1, device=torch.device("cpu"))
...
@pytest.mark.dqc
def test_dftxc_eval_cuda():
    inputs = 'deepchem/models/tests/assets/test_dftxcdata.yaml'
    data = DFTYamlLoader()
    model_dir = tempfile.mkdtemp()
    dataset = (data.create_dataset(inputs, device=torch.device("cuda")))
    model = XCModel("lda_x",
                    batch_size=1,
                    log_frequency=1,
                    mode="classification",
    ...
    reload_model.restore()
    inputs1 = 'deepchem/models/tests/assets/test_ieLi.yaml'
    predict_dataset = data.create_dataset(inputs1, device=torch.device("cuda"))
    predict = reload_model.predict(predict_dataset)
    assert predict < 0.199
    metric = dc.metrics.Metric(dc.metrics.mae_score)
    scores = model.evaluate(dataset, [metric])
...
def test_dm_cuda():
    ...
    data = DFTYamlLoader()
    dataset = (data.create_dataset(inputs))
    model = XCModel("lda_x", batch_size=1, device="cuda")
    loss = model.fit(dataset, nb_epoch=1, checkpoint_interval=1)
    assert loss < 0.008
