...
def test_hybridxc():
    ...
    n = 2
    nnmodel = dummymodel()
    k = HybridXC("lda_x", nnmodel, aweight0=0.0, device="cuda:0")
    densinfo = ValGrad(
        value=torch.rand((n,), dtype=torch.float32).requires_grad_())
    output = k.get_edensityxc(densinfo)
    expected_output = torch.tensor([-0.6988, -0.2108],
                                   dtype=torch.float64).to("cuda:0")
    torch.testing.assert_close(output, expected_output, atol=1e-4, rtol=0)
