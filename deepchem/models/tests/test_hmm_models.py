import numpy as np
from deepchem.models.hmm_models import (MultinomialHMMWrapper,
                                        CategoricalHMMWrapper,
                                        PoissonHMMWrapper, GaussianHMMWrapper,
...
if dataset_name == 'tox21':
    ...
    dataset = datasets[0]  # Use training set
...
def verify_model_with_dataset(model, X, lengths, dataset=None):
    ...
    if dataset is not None:
        assert X.shape[1] == dataset.X.shape[1], "Feature dimensions should match"
        ...
        if len(dataset.X) > 10:
            ...
            model_split = type(model)(n_components=model.n_components,
...
def test_hmm_with_tox21_dataset():
    ...
    try:
        X, lengths, dataset = create_molecular_sequence_dataset('tox21', max_molecules=50)
...
def test_hmm_with_delaney_dataset():
    ...
    try:
        X, lengths, dataset = create_molecular_sequence_dataset('delaney', max_molecules=30)

        model = VariationalGaussianHMMWrapper(n_components=2, n_iter=10, random_state=42)
        verify_model_with_dataset(model, X, lengths, dataset)

        print(f"Successfully tested with Delaney dataset: {X.shape[0]} samples, "
              f"{X.shape[1]} features")
