...
@pytest.mark.torch
def test_gcn_fit_predict_workflow(gcn_data):
    """
...
@pytest.mark.torch
def test_gcn_checkpointing_and_loading(gcn_data, tmp_path="temp"):
    """
    ...
    gcn_model_new = dc.models.GCNModel(
        mode='classification',
        n_tasks=len(tasks),
        number_atom_features=30,
        batch_size=10,
    ...
    reloaded_model = DCLightningModule.load_from_checkpoint(
        ckpt_path, dc_model=gcn_model_new)
