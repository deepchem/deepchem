...
class PretrainedKerasModel(Model):
  ...
  def __init__(self,
    ...
    """
    super(PretrainedKerasModel, self).__init__(
        model_instance=model, model_dir=model_dir, **kwargs)
    if restore_from is None:
      self.restore_from = model_dir
    else:
      if not os.path.exists(restore_from):
        raise DirectoryNotFoundError(
            "{} not found for loading pretrained model.".format(restore_from))
  ...
  def _create_saver(self, max_checkpoints_to_keep=5, var_list=None):
    if self._saver_built:
      return
    self._saver = tf.train.Saver(
        max_to_keep=max_checkpoints_to_keep, var_list=var_list)
    self._saver_built = True
  ...
  def fit_generator(self,
    ...
    for batch in generator:
      ...
      if checkpoint_interval > 0:
        self._create_saver(max_checkpoints_to_keep)
      if restore:
        self.restore()
      ...
      if tf.executing_eagerly():
        ...
        with tf.GradientTape() as tape:
          ...
          if isinstance(outputs, tf.Tensor):  # if checkpoint_interval > 0:
      ...
      if checkpoint_interval > 0 and current_step % checkpoint_interval == checkpoint_interval - 1:
        model_name = "model-" + str(current_step) + ".ckpt"
  ...
  def restore(self, checkpoint=None):
    ...
    if self._restored_model:
      return
    self._ensure_built()
    self._create_saver(var_list=None)
    if checkpoint is None:
      checkpoint = tf.train.latest_checkpoint(self.restore_from)
    if checkpoint is None:
      raise ValueError('No checkpoint found')
    if tf.executing_eagerly():
      self._saver.restore(checkpoint)
    else:
      self._saver.restore(sess=self.session, save_path=checkpoint)
    self._restored_model = True

  def load_pretrained(self, include_top=True, checkpoint=None):
    """Reload the values of all variables from a checkpoint file.
    Parameters
    ----------
    include_top: bool, default True
        Whether to include the dense layers in the end
    checkpoint: str
      the path to the checkpoint file to load.  If this is None, the most recent
      checkpoint will be chosen automatically.  Call get_checkpoints() to get a
      list of all available checkpoints.
    ...
    if include_top:
      ...
      self._ensure_built()
      self._create_saver(var_list=var_list)

      if tf.executing_eagerly():
        raise ValueError("Pretrained restore not supported yet in saver.")
      else:
        self._saver.restore(sess=self.session, save_path=checkpoint)
