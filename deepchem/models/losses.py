...
class MolGraphConvMILoss():
    """
    Losses used to maximize the mutual information between molecular graph encodings.
    Encodings can be generated from molecular graphs using a graph convolutional network, and the mutual information is calculated between subgraphs (local encodings) and the entire molecule (global encodings) in order to enable unsupervised training.

    This class supports only PyTorch.
    ...
    def __init__(self, device, measure='JSD', average_loss=True):
        self.device = device
        self.measure = measure
        self.average_loss = average_loss


class local_global_graphconv_loss():
    """
    Compares the local and global encodings of molecular graphs and returns the loss between them based on the measure specified.
    Global in this case refers to the global encoding of the entire molecule, and local refers to the encoding of subgraphs.
    ...
    def _create_pytorch_loss(self, l_enc, g_enc, index):
        ...
        E_pos = self.get_positive_expectation(res * pos_mask, self.measure,
                                              self.average_loss)
        E_pos = (E_pos * pos_mask).sum() / pos_mask.sum()
        E_neg = self.get_negative_expectation(res * neg_mask, self.measure,
                                              self.average_loss)
...
class global_global_loss(Loss):
    ...
    def _create_pytorch_loss(self, g_enc, g_enc1):
