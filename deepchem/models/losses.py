...
class MolGraphConvMILoss():
    """
    Losses used to maximize the mutual information between molecular graph encodings. Encodings can be generated from molecular graphs using a graph convolutional network, and the mutual information is calculated between subgraphs (local encodings) and the entire molecule (global encodings) in order to enable unsupervised training.
    ...
    def local_global_loss(self, l_enc, g_enc, index):
        """
        Compares the local and global encodings of molecular graphs and returns the loss between them based on the measure specified. Global in this case refers to the global encoding of the entire molecule, and local refers to the encoding of subgraphs.
