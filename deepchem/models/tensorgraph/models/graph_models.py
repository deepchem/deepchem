...
class GraphConvTensorGraph(TensorGraph):
    ...
    def build_graph(self):
        ...
        for layer_size in self.graph_conv_layers:
            ...
            in_layer = GraphPool(in_layers=[batch_norm1, self.degree_slice, self.membership]
                                           + self.deg_adjs)
        dense = Dense(out_channels=self.dense_layer_size, activation_fn=tf.nn.relu, in_layers=[in_layer])
