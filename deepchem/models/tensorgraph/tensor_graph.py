...
class TensorGraph(Model):
    ...
    def _add_layer(self, layer):
        if layer.name is None:
            layer.name = "%s_%s" % (
                layer.__class__.__name__, len(self.layers) + 1)
        ...
        with self._get_tf("Graph").as_default():
            with tf.Session() as sess:
                ...
                for feed_dict in generator:
                    ...
                    result = np.array(
                        sess.run(out_tensors, feed_dict=feed_dict))
            ...
            for node in order:
                with tf.name_scope(node):
                    ...
                    node_layer.create_tensor(
                        training=self._training_placeholder)
    ...
    def _get_tf(self, obj):
        ...
        if obj == "Graph":
            ...
            self.tensor_objects[
                'FileWriter'] = tf.summary.FileWriter(self.model_dir)
        elif obj == 'Optimizer':
