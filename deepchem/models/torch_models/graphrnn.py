...
class _GraphRNNTorchModel(nn.Module):
    ...
    def __init__(self, node_input_dim, node_hidden_dim, node_feature_dim,
                 edge_input_dim, edge_hidden_dim, edge_feature_dim, num_neighbors):
        super(_GraphRNNTorchModel, self).__init__()
        self.node_mlp = NodeFeatureMLP(node_input_dim, node_hidden_dim, node_feature_dim)
        self.edge_rnn = EdgeRNN(edge_input_dim, edge_hidden_dim, edge_feature_dim)
    ...
    def forward(self, inputs):
        ...
        generated_nodes = []
        generated_edges = []
        h_edge = torch.zeros(1, batch_size, self.edge_hidden_dim, device=node_seq.device)
        ...
        return torch.stack(generated_nodes, dim=1), torch.stack(generated_edges, dim=1)
...
class GraphRNN(TorchModel):
    ...
    def __init__(self, node_input_dim, node_hidden_dim, node_feature_dim,
                 ...
                 learning_rate=0.001, num_neighbors=5, mode='regression', n_classes=2):
        self.mode = mode
        self.n_classes = n_classes
        model = _GraphRNNTorchModel(node_input_dim, node_hidden_dim, node_feature_dim,
                                    edge_input_dim, edge_hidden_dim, edge_feature_dim, num_neighbors)
        loss = L2Loss() if mode == 'regression' else SoftmaxCrossEntropy()
        super(GraphRNN, self).__init__(model, loss=loss, learning_rate=learning_rate)
    ...
    def default_generator(self, dataset, epochs=1, mode='fit', deterministic=True, pad_batches=True):
        ...
        for epoch in range(epochs):
            for g_b, y_b, w_b, _ in dataset.iterbatches(batch_size=self.batch_size,
                                                         deterministic=deterministic,
                                                         pad_batches=pad_batches):
                # g_b is a list of GraphData objects.
                # Extract node features; each g.node_features is (num_nodes, node_input_dim)
                node_seq_list = [torch.tensor(g.node_features, dtype=torch.float32) for g in g_b]
                ...
                for ns in node_seq_list:
                    if ns.shape[0] < max_num_nodes:
                        pad = torch.zeros(max_num_nodes - ns.shape[0], ns.shape[1], dtype=torch.float32)
                        ns = torch.cat([ns, pad], dim=0)
                    node_seq_padded.append(ns)
                node_seq = torch.stack(node_seq_padded, dim=0)  # (B, max_num_nodes, node_input_dim)
                
                # Extract edge features; assume each g.edge_features is (num_nodes, num_neighbors, edge_input_dim)
                edge_seq_list = [torch.tensor(g.edge_features, dtype=torch.float32) for g in g_b]
                edge_seq_padded = []
                for es in edge_seq_list:
                    if es.shape[0] < max_num_nodes:
                        pad = torch.zeros(max_num_nodes - es.shape[0], es.shape[1], es.shape[2], dtype=torch.float32)
                        es = torch.cat([es, pad], dim=0)
                    edge_seq_padded.append(es)
                edge_seq = torch.stack(edge_seq_padded, dim=0)  # (B, max_num_nodes, num_neighbors, edge_input_dim)
                
                # Handle weights if provided
                w_tensor = torch.tensor(w_b, dtype=torch.float32) if w_b is not None else None
                # Convert targets
