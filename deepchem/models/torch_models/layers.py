...
class LocalMessagePassing(torch.nn.Module):
    def __init__(self, config):
        ...
        self.mlp = MultilayerPerceptron(d_input=self.dim, d_output=self.dim, activation_fn='silu')

        self.mlp_kj = MultilayerPerceptron(d_input=3*self.dim, d_output=self.dim, activation_fn='silu')
        self.mlp_sbf = MultilayerPerceptron(d_input=self.dim, d_hidden=(self.dim,), d_output=self.dim, activation_fn='silu')
    
        self.lin_rbf = nn.Linear(self.dim, self.dim, bias=False)

        self.y_mlp = MultilayerPerceptron(d_input=self.dim, d_hidden=(self.dim, self.dim,), d_output=self.dim, activation_fn='silu')
        self.y_W = nn.Linear(self.dim, 1)

    def Res(self, m):
        self.mlp_res = MultilayerPerceptron(d_input=self.dim, d_hidden=(self.dim,), d_output=self.dim, activation_fn='silu')
        m1 = self.mlp_res(m)
        m_out = m1 + m
        return m_out
