...
class KFAC_Optimizer(optim.Optimizer):
  """"
  ...
  def ComputeCovA(self, a: torch.Tensor,
  ...
  def ComputeCovG(self, g: torch.Tensor,
                  layer: torch.nn.Module) -> torch.Tensor:
    """
  ...
  def _save_grad_output(self, module: torch.nn.Module, grad_input: torch.Tensor,
    ...
    if self.steps % self.TCov == 0:
      gg = self.ComputeCovG(grad_output[0].data, module)
      # Initialize buffers
      if self.steps == 0:
