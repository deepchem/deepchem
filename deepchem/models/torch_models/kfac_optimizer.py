...
class KFACOptimizer(optim.Optimizer):
    ...
    def _extract_patches(x, kernel_size:List[int], stride:List[int], padding:List[int]):
        """
    ...
    def _update_inv(self, m):
        ...
        if self.mean:
                self.d_a[m], self.Q_a[m] = torch.symeig(self.m_aa[m]-torch.mean(self.m_aa[m]), eigenvectors=True)
                self.d_g[m], self.Q_g[m] = torch.symeig(self.m_gg[m]-torch.mean(self.m_gg[m]), eigenvectors=True)
        else:
