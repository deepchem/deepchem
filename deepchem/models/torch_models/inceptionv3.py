...
class InceptionV3(nn.Module):
    def __init__(self, input_shape, num_classes=1000, aux_logits=True, dropout_rate=0.5):
        super(InceptionV3, self).__init__()
        self.aux_logits = aux_logits
        self.input_shape = input_shape

        # Initial layers
        self.Conv2d_1a_3x3 = BasicConv2d(input_shape[0], 32, kernel_size=3, stride=2)
...
class InceptionV3Model(TorchModel):
    ...
    def __init__(self, input_shape=(6, 100, 221), **kwargs):
        ...
        model = InceptionV3(num_classes=3, input_shape=input_shape, aux_logits=False) # Set aux_logits to False to disable auxiliary output
        ...
        super(InceptionV3Model, self).__init__(model=model, optimizer=optimizer, loss=loss, **kwargs)
    ...
    def adjust_learning_rate(self):
        ...
        if self.current_step < self.warmup_steps:
            ...
            decay_factor = self.decay_rate ** (self.current_step // self.decay_steps)
            lr = self.learning_rate * decay_factor

        self.optimizer.learning_rate = lr


    def fit(self, dataset: ImageDataset, **kwargs):
        """
        ...
        dataset: ImageDataset
            DeepChem ImageDataset to be used for training.
        """
        nb_epoch = kwargs.pop('nb_epoch', 1)  # Use 1 as the default value if nb_epoch is not provided
        ...
        for epoch in range(nb_epoch):
            ...
            super(InceptionV3Model, self).fit(dataset, nb_epoch=1, **kwargs)  # Train for one epoch at a time
