...
def test_IRV_multitask_classification_reload():
    ...
    reloaded_model = dc.models.torch_models.MultitaskIRVClassifier(n_tasks,
                                                      K=5,
                                                      learning_rate=0.01,
                                                      batch_size=n_samples,
                                                      model_dir=model_dir)
...
@pytest.mark.torch
def compare_tf_and_torch_output():
    """Test IRV classifier overfits tiny data."""
    n_tasks = 5
    n_samples = 10
    n_features = 128
    ...
    model_torch=dc.models.torch_models.MultitaskIRVClassifier(n_tasks,
                                             K=5,
                                             learning_rate=0.01,
                                             batch_size=n_samples)    
    ...
    torch_output=model_torch.predict(dataset= dataset_trans,transformers=[IRV_transformer])
    
    # Compare pytorch and tensorflow outputs
    assert np.allclose(tf_output, torch_output,rtol=1e-5, atol=1e-6), "Outputs do not match!"