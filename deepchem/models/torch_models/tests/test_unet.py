...
def test_UNetModel():
    ...
    input_samples = np.random.randn(5, 3, 10, 10).astype(np.float32)   # 5 RGB 10x10 pixel input images
    output_samples = np.random.randn(5, 1, 10, 10).astype(np.float32)  # 5 grey scale 10x10 pixel output segmentation masks
...
def test_restore_UNetModel():
    ...
    reloaded_model = UNetModel(in_channels=3, out_channels=1, model_dir=model_dir)
