...
def synthetic_dataset():
    ...
    for _ in range(batch_size):
        ...
        edge_features = np.random.randn(num_nodes, num_neighbors,
                                        edge_feature_dim)
        # Create a simple adjacency list: each node i lists its next num_neighbors nodes (with wrap-around)
        adjacency_list = {
            i: [(i + j + 1) % num_nodes for j in range(num_neighbors)
...
def test_forward_pass(synthetic_dataset):
    ...
    assert node_preds.shape[
        2] == node_feature_dim, f"Node feature dim should be {node_feature_dim}."
    ...
    assert edge_preds.shape[
        2] == num_neighbors, f"Number of neighbors should be {num_neighbors}."
    assert edge_preds.shape[
        3] == edge_feature_dim, f"Edge feature dim should be {edge_feature_dim}."
...
def test_metric_evaluation(synthetic_dataset):
    ...
    assert isinstance(scores['mean_absolute_error'],
                      float), "Metric value should be a float."
