...
def test_lnnmodel_overfit():
    ...
    # inputs -> [x, theta, x_dot, theta_dot]
    inputs = np.array(
        [[0.1681, -0.1578, 0.0100, 0.1712], [0.2179, -0.1347, 0.9779, 0.2733],
         [0.3656, -0.1052, 1.9439, 0.2995], [0.6101, -0.0764, 2.8944, 0.2663],
         [0.9493, -0.0522, 3.8154, 0.2109], [1.3796, -0.0336, 4.6957, 0.1574]],
        dtype=np.float32)

    # labels -> [x_ddot, theta_ddot]
    labels = np.array([[9.5541, 1.3175], [9.5940, 0.6434], [9.5125, -0.0977],
                       [9.2853, -0.4924], [8.9339, -0.5630], [8.4838, -0.4825]],
                      dtype=np.float32)
