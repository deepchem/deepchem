...
class FourierBlock1D(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, modes: int):
        ...
        print(f"[FourierBlock1D] Initialized with weights shape: {self.weights.shape}")

    def compl_mul1d(self, a, b):
        real = torch.einsum("bci,oci->boi", a[..., 0], b[..., 0]) - torch.einsum(
            "bci,oci->boi", a[..., 1], b[..., 1]
        )
        imag = torch.einsum("bci,oci->boi", a[..., 0], b[..., 1]) + torch.einsum(
            "bci,oci->boi", a[..., 1], b[..., 0]
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out_ft = torch.zeros(B, self.out_channels, x_ft.shape[-2], 2, device=x.device)
        limit = min(self.modes, x_ft.shape[-2])
        out_ft[:, :, :limit] = self.compl_mul1d(
            x_ft[:, :, :limit], self.weights[:, :, :limit]
        )
...
class PINO(TorchModel):
    ...
    def parameters(self):
        all_params = list(self.model.parameters()) + list(self.phys_params.values())
        print(f"[PINO] Total parameters count: {len(all_params)}")
        return all_params
    ...
    class FNO1D(nn.Module):
        def __init__(self, in_channels, param_dim, modes, width):
            super().__init__()
            self.param_encoder = (
                nn.Linear(param_dim, width // 2) if param_dim > 0 else None
            )
            fc0_in_dim = in_channels + (
                width // 2 if self.param_encoder is not None else 0
            )
        ...
        def forward(self, x: torch.Tensor, params: torch.Tensor = None) -> torch.Tensor:
            ...
            if self.param_encoder is not None:
                if params is None:
                    params = torch.zeros(
                        x.shape[0], self.param_encoder.in_features, device=x.device
                    )
    ...
    def _create_hi_res_inputs(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        print(f"[PINO] Created hi_res_inputs with shape: {hi_res_inputs.shape}")
    ...
    def fit(self, dataset, nb_epoch=10, **kwargs):
        ...
        optimizer = torch.optim.Adam(
            [
                ...
                {"params": self.phys_params.values(), "lr": phys_lr},
            ]
        )
        grad_clip_threshold = 0.5
        device = self.parameters()[0].device  # noqa: F841
    ...
    def _prepare_batch(self, batch):
        if isinstance(batch, list) and isinstance(batch[0], dict):
            ...
            print(f"[_prepare_batch] Batch provided as tuple of length {len(batch)}.")
            inputs = batch[0]
        ...
        try:
            print("DEBUG: np.shape(inputs):", np.shape(inputs))
        except Exception as e:
            print("DEBUG: np.shape failed:", e)
        if isinstance(inputs, np.ndarray):
            print("DEBUG: Detected inputs as numpy array; shape:", inputs.shape)
            inputs = torch.from_numpy(inputs).float()
        elif not isinstance(inputs, torch.Tensor):
            try:
                inputs = torch.tensor(inputs, dtype=torch.float32)
                print("DEBUG: Converted inputs to tensor; shape:", inputs.shape)
            except Exception as e:
                print("Error converting inputs to torch tensor:", e)
                raise
        if inputs.ndim == 2:
            print("DEBUG: inputs has 2 dimensions; unsqueezing to get (B, 1, D)")
            inputs = inputs.unsqueeze(1)
            print("DEBUG: New inputs shape:", inputs.shape)
        if labels is not None and not isinstance(labels, torch.Tensor):
            if isinstance(labels, np.ndarray):
                print("DEBUG: Detected labels as numpy array; shape:", labels.shape)
                labels = torch.from_numpy(labels).float()
            else:
                try:
                    labels = torch.tensor(labels, dtype=torch.float32)
                    print("DEBUG: Converted labels to tensor; shape:", labels.shape)
                except Exception as e:
                    print("Error converting labels to torch tensor:", e)
                    raise
        ...
        if self._is_training and self.hi_res:
            ...
            self._hi_res_inputs = torch.randn(
                32, self.hi_res[0], 2, device=inputs.device
            )
        return inputs, labels, weights

    def _compute_pde_loss(
        self, inputs: torch.Tensor, _unused: torch.Tensor
    ) -> torch.Tensor:
        print("[_compute_pde_loss] Computing PDE loss...")
        ...
        if grads is None:
            print("[_compute_pde_loss] Warning: grads is None, returning PDE loss = 0")
            return torch.tensor(0.0, device=inputs.device, dtype=inputs.dtype)
        print(f"[_compute_pde_loss] grad shape: {grads.shape}")
        u_x = grads[..., 0:1]
        u_t = grads[..., 1:2]
        print(f"[_compute_pde_loss] u_x shape: {u_x.shape}, u_t shape: {u_t.shape}")
        ...
        print(f"[_compute_pde_loss] u_xx shape: {u_xx.shape}")
        # Use the reparameterized nu: nu = exp(alpha)
        alpha = (
            self.phys_params["alpha"]
            if "alpha" in self.phys_params
            else torch.tensor(np.log(0.01), device=inputs.device, requires_grad=True)
        )
        nu = torch.exp(alpha)
        print(f"[_compute_pde_loss] Using nu = exp(alpha) value: {nu.item()}")
        residual = u_t + u * u_x - nu * u_xx
        if torch.isnan(residual).any() or torch.isinf(residual).any():
            print("[_compute_pde_loss] Warning: NaN or Inf in PDE residual!")
            return torch.tensor(1e6, device=inputs.device, dtype=inputs.dtype)
        pde_loss = torch.mean(residual**2) * 10000000000
        print(f"[_compute_pde_loss] PDE loss: {pde_loss.item()}")
        print(
        ...
        print(f"[_compute_pde_loss] nu value (exp(alpha)): {nu.item()}")
        print(f"[_compute_pde_loss] Calculated PDE loss: {pde_loss.item()}")
        return pde_loss

    def _compute_boundary_loss(self) -> torch.Tensor:
        print("[_compute_boundary_loss] Computing boundary loss...")
    ...
    def _compute_physics_loss(
        self, inputs: torch.Tensor, u: torch.Tensor
    ) -> torch.Tensor:
            ...
            for batch in dataset.iterbatches():
                ...
                if params is not None:
                    params_tensor = torch.as_tensor(
                        params, dtype=torch.float32, device=inputs.device
                    )
                    print(
                    ...
                    params_tensor = None
                    print("[predict] No params provided.")
                output = self.model(inputs, params_tensor)
                print(f"[predict] Output shape: {output.shape}")
                preds.append(output)
        final_preds = torch.cat(preds, dim=0)
        print(f"[predict] Final predictions shape: {final_preds.shape}")
        return final_preds
