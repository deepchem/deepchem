    ...
    class MAML(object):
        ...
        def __init__(
            ...
            self.learning_rate: Union[float,
                                      LearningRateSchedule] = learning_rate
            ...
            for i in range(steps):
                ...
                for j in range(self.meta_batch_size):
                    ...
                    meta_loss, _ = self.learner.compute_model(
                        learner.get_batch(), updated_variables, True)
                    meta_gradients: Tuple[
                        torch.Tensor, ...] = torch.autograd.grad(
                            meta_loss,
                            variables,
                            grad_outputs=torch.ones_like(meta_loss),
                            retain_graph=True)
                    if j == 0:
                        summed_gradients: Union[
                            Tuple[torch.Tensor,
                                  ...], List[torch.Tensor]] = meta_gradients
                    else:
                        summed_gradients = [
                            s + g
                            for s, g in zip(summed_gradients, meta_gradients)
                        ]
                    ind: int = 0
        ...
        def predict_on_batch(
            ...
            outputs is a list of the model's outputs
            """
            return self.learner.compute_model(inputs, self.learner.variables,
                                              False)
