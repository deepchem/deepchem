...
from deepchem.feat import hydrogenate_and_compute_partial_charges, rdkit_grid_featurizer
from deepchem.feat.nnscore_utils import rdkit_hydrogenate_and_compute_partial_charges
...
class VinaPoseGenerator(PoseGenerator):
  ...
  def generate_poses(self, protein_file, ligand_file,
    ...
    if not dry_run:
      self.call_vina(conf_file, log_file, out_pdbqt)

    # Return docked files 
    return protein_hyd, out_pdbqt

  def rdkit_generate_poses(self, protein_file, ligand_file,
                           centroid=None, box_dims=None,
                           dry_run=False, out_dir=None):
    """Generates the docked complex and outputs files for docked complex."""
    if out_dir is None:
    ...
    protein_hyd = os.path.join(out_dir, "%s.pdb" % receptor_name)
    protein_pdbqt = os.path.join(out_dir, "%s.pdbqt" % receptor_name)
    rdkit_hydrogenate_and_compute_partial_charges(protein_file, "pdb",
                                            hyd_output=protein_hyd,
                                            pdbqt_output=protein_pdbqt,
                                            protein=True)
    # Get protein centroid and range
    receptor_mol = rdkit_grid_featurizer.load_molecule(protein_hyd, add_hydrogens=False)
    # TODO(rbharath): Need to add some way to identify binding pocket, or this is
    # going to be extremely slow!
    if centroid is not None and box_dims is not None:
      protein_centroid = centroid
  ...
  def call_vina(self, conf_file, log_file, out_pdbqt):
