...
from typing import Any, Iterator, List, Optional, Tuple, Union, cast, IO, Generator
...
def chunkify(a: torch.Tensor, dim: int, maxnumel: int) -> \
    ...
    --------
    >>> import torch
    >>> from deepchem.utils.data_utils import chunkify
    >>> a = torch.arange(10)
...
def get_memory(a: torch.Tensor) -> int:
    ...
    --------
    >>> import torch
    >>> from deepchem.utils.data_utils import get_memory
...
def get_dtype_memsize(a: torch.Tensor) -> int:
    ...
    --------
    >>> import torch
    >>> from deepchem.utils.data_utils import get_dtype_memsize
    >>> a = torch.randn(100, 100, dtype=torch.float64)
    >>> get_dtype_memsize(a)
    8
