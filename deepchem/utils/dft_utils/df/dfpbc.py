...
class DFPBC(BaseDF):
    ...
    def build(self) -> BaseDF:
        ...
        def _calc_integrals():
            ...
            j3c_short_f = pbc_coul3c(_basisw, other2=_fusew, kpts_ij=kpts_ij,
            ...
            j2c_short_f = pbc_coul2c(fuse_aux_wrapper, kpts=kpts_reduce,
                                           options=self._lattsum_opt)
            ...
            comp_ft = eval_gto_ft(aux_comp_wrapper, gvk)  # (nxcao, ngv * nkpts_ij)
            ...
            olp_mat = pbc_overlap(self._wrapper, kpts=self._kpts,
    ...
    def _create_compensating_bases(self, atombases: List[AtomCGTOBasis], eta: float) -> List[AtomCGTOBasis]:
        ...
        norms = [half_sph_norm / gaussian_integral(2 * angmom + 2, eta) for angmom in range(7)]
