...
Pure Python/NumPy port of the libcint optimizer pipeline for both
1-electron and 2-electron integrals.

Entry points
------------
    int1e_ovlp_optimizer(atm, natm, bas, nbas, env) -> CINTOpt
    int2e_ar12b_optimizer(opt_ref, atm, natm, bas, nbas, env) -> CINTOpt
...
env layout:
    env[0]  PTR_EXPCUTOFF — 0.0 means use default EXPCUTOFF=60
    env[PTR_COORD]        — atom coordinates, 3 doubles per atom
    env[PTR_EXP]          — primitive exponents
...
@dataclass
class PairData:
    """Precomputed data for one (ip, jp) primitive pair."""
    rij:   np.ndarray   # shape (3,) — weighted centre-of-charge
    eij:   float        # exp(-eij) overlap factor
    cceij: float        # screening exponent (eij - log_rr - log_maxc terms)
...
class CINTOpt:
    ...
    """
    nbas:            int
    index_xyz_array: Optional[list] = None  # indexed by i*LMAX1+j (1e) or i*L^3+... (2e)
    non0ctr:         Optional[list] = None  # (nprim,)      int32, one per shell
    sortedidx:       Optional[list] = None  # (nprim, nctr) int32, one per shell
    log_max_coeff:   Optional[list] = None  # (nprim,)      float64, one per shell
    pairdata:        Optional[list] = None  # flat list length nbas*nbas
...
def approx_log(x: float) -> float:
    ...
    Accurate to the nearest power of 2; exact only at powers of 2.
    Max error ln(2) ≈ 0.693. Matches C union trick bit-for-bit.
    """
    bits = np.float64(x).view(np.uint64)
    bits = int(bits)
    return (int((bits >> 52) & 0x7FF) - 1023 + 1) * 0.693145751953125
...
def CINTcart_comp(lmax: int) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Cartesian component exponents (nx, ny, nz) for angular momentum lmax,
    in libcint canonical order (decreasing nx, then decreasing ny).
    Returns three int32 arrays of shape ((lmax+1)*(lmax+2)//2,).
    """
    nf = (lmax + 1) * (lmax + 2) // 2
    ...
    for lx in range(lmax, -1, -1):
        for ly in range(lmax - lx, -1, -1):
            nx[inc] = lx;  ny[inc] = ly;  nz[inc] = lmax - lx - ly
            inc += 1
    return nx, ny, nz
...
def _numpy_vec_log_maxc(log_maxc: np.ndarray, coeff: np.ndarray,
    ...
    """Vectorised CINTOpt_log_max_pgto_coeff — approx_log of max|coeff| per primitive."""
    c    = np.asarray(coeff, dtype=np.float64).reshape(ictr, nprim)
    maxc = np.abs(c).max(axis=0)
...
def CINTOpt_set_log_maxc(opt: CINTOpt, atm: np.ndarray, natm: int,
    ...
    """
    Populate opt.log_max_coeff — approx_log of max|coeff| for each primitive.
    Single contiguous buffer; per-shell entries are views into it.
    """
    nprims   = bas[:nbas, NPRIM_OF].astype(int)
    tot_prim = int(nprims.sum())
    if tot_prim == 0:
        return
    plog = np.empty(tot_prim)
    opt.log_max_coeff = []
    offset = 0
    for i in range(nbas):
        ip = nprims[i];  ic = int(bas[i, NCTR_OF])
        slc = plog[offset : offset + ip]
        _numpy_vec_log_maxc(slc, env[bas[i, PTR_COEFF] : bas[i, PTR_COEFF] + ip * ic], ip, ic)
...
def CINTOpt_non0coeff_byshell(ci: np.ndarray, iprim: int,
    ...
    """
    For each primitive, sort contraction indices: non-zeros first, zeros after.
    Returns sortedidx (iprim, ictr) int32 and non0ctr (iprim,) int32.
    """
    c     = ci.reshape(ictr, iprim)
    mask  = (c != 0)
    order = np.argsort(~mask, axis=0, kind='stable')
    return order.T.astype(np.int32), mask.sum(axis=0).astype(np.int32)
...
def _make_fakebas(bas: np.ndarray, nbas: int) -> tuple[np.ndarray, int]:
    """Minimal fake basis: one shell per l=0..max_l with only ANG_OF set."""
...
def CINTinit_int1e_EnvVars(envs: CINTEnvVars, ng: np.ndarray, shls: np.ndarray,
                            atm: np.ndarray, natm: int,
                            bas: np.ndarray, nbas: int, env: np.ndarray) -> None:
    """Populate CINTEnvVars for a 1e integral over shell pair (shls[0], shls[1])."""
    ...
    envs.x_ctr = np.array([bas[i_sh, NCTR_OF], bas[j_sh, NCTR_OF], 0, 0], dtype=np.int32)
...
def CINTg1e_index_xyz(envs: CINTEnvVars) -> np.ndarray:
    """g-array index map for a 1e shell pair. Returns (nf*3,) int32."""
...
def CINTinit_int2e_EnvVars(envs: CINTEnvVars, ng: np.ndarray, shls: np.ndarray,
    ...
    envs.common_factor = (math.pi ** 3) * 2 / SQRTPI \
...
def gen_idx(opt: CINTOpt, finit, findex_xyz, order: int, max_l_override: int,
            ng: np.ndarray, atm: np.ndarray, natm: int,
            bas: np.ndarray, nbas: int, env: np.ndarray) -> None:
    """
    Pre-compute g-array index maps for all shell angular momentum combinations.
    Stored in opt.index_xyz_array, indexed by i*LMAX1+j (order=2) or
    i*LMAX1^3 + j*LMAX1^2 + k*LMAX1 + l (order=4).
    """
...
def CINTinit_2e_optimizer(atm: np.ndarray, natm: int,
    ...
    """Allocate a blank CINTOpt (all fields None). Mirrors C malloc+NULL init."""
...
def CINTall_1e_optimizer(opt: CINTOpt, ng: np.ndarray,
                          ...
                          bas: np.ndarray, nbas: int,
                          env: np.ndarray) -> None:
    """Populate all optimizer tables for a 1e integral."""
    CINTOpt_set_log_maxc(opt, atm, natm, bas, nbas, env)
...
def int1e_ovlp_optimizer(opt_ref,
                          atm: np.ndarray, natm: int,
                          bas: np.ndarray, nbas: int,
                          env: np.ndarray) -> CINTOpt:
    """
    1e overlap optimizer. Matches C signature:
