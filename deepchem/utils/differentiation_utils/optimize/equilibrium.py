...
from deepchem.utils.differentiation_utils import ConvergenceWarning


def anderson_acc(fcn: Callable[..., torch.Tensor], x0: torch.Tensor, params: List,
                 feat_ndims: int = 1,
    ...
    for k in range(2, maxiter):
        ...
        if verbose:
            if k < 10 or k % 10 == 0 or to_stop:
                print("%6d: |dx|=%.3e, |f-x|=%.3e" % (k, (xnew - xn).norm(), (fnew - xnew).norm()))
        # update the xn
        xn = xnew
        if to_stop:
            converge = True
