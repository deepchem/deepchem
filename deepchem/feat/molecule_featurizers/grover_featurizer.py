...
class GroverFeaturizer(MolecularFeaturizer):
    ...
    def _make_mol_graph(self, mol: RDKitMol) -> GraphData:
        ...
        edge_index = np.asarray(
            edge_index).T  # to make it as shape (2, num_edges)
    ...
    def _featurize(self, mol: RDKitMol, **kwargs) -> GraphData:
        ...
        """
        molgraph = self._make_mol_graph(mol)
        setattr(molgraph, 'fg_labels',
                self.functional_group_generator.featurize(mol)[0])
        if self.featurizer:
            setattr(molgraph, 'additional_features',
                    self.featurizer.featurize(mol)[0])
        return molgraph
