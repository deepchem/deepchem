...
try:
    import torch
    from deepchem.feat.dqc_gpu import *
except:
    pass
...
class DFTSystem():
    ...
    def __init__(self, system: Dict, device: torch.tensor):
        self.device = device
        self.system = system
    ...
    def get_dqc_mol(self, pos_reqgrad: bool = False) -> BaseSystem:
        ...
        if (self.device == torch.device('cuda')):
            molecule = mol(self.moldesc,
                           self.basis,
                           spin=self.spin,
                           charge=self.charge,
                           device="cuda")
        else:
            molecule = dqc.Mol(self.moldesc,
                               self.basis,
                               spin=self.spin,
                               charge=self.charge)
        return molecule
...
class DFTEntry():
    ...
    def __init__(self,
                 ...
                 systems: List[Dict],
                 weight: Optional[int] = 1):
        self._systems = [DFTSystem(p, device=self._device) for p in systems]
        self._weight = weight
