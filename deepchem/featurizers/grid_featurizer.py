        ...
        if np.abs(atom.GetFormalCharge() - 1.0) < 0.01 or '+' in atom.GetType():
          ...
          if _is_cation_pi(cation_position, protein_ring_center, protein_ring_normal):
            protein_cation_pi = _update_feature_dict(protein_cation_pi, 
                                                    idxs=protein_ring._path)
            ligand_cation_pi = _update_feature_dict(ligand_cation_pi,
...
def _compute_binding_pocket_cation_pi(protein_xyz, protein, ligand_xyz, ligand):
  ...
  (protein_cation_pi, ligand_cation_pi) = _compute_cation_pi(protein, ligand, protein_cation_pi,
                                                          ligand_cation_pi)
  (ligand_cation_pi, protein_cation_pi) = _compute_cation_pi(ligand, protein, ligand_cation_pi,
                                                          protein_cation_pi)
  return (protein_cation_pi, ligand_cation_pi)
...
def _compute_hydrogen_bonds(protein_xyz, protein, ligand_xyz, ligand, 
  ...
  for i, hbond_dist_bin in enumerate(hbond_dist_bins):
    ...
    hbond_contacts.append(_compute_hbonds_in_range(protein, protein_xyz, ligand, 
...
class GridFeaturizer(ComplexFeaturizer):
  ...
  def _transform(self, protein_pdb, ligand_pdb):
    ...
    if "ecfp" in self.voxel_feature_types:
      protein_ecfp_dict, ligand_ecfp_dict = _featurize_binding_pocket_ecfp(protein_xyz, protein_ob, ligand_xyz,
                                          ligand_ob, pairwise_distances, cutoff=4.5, ecfp_degree=self.ecfp_degree)
    if "splif" in self.voxel_feature_types: 
      splif_dicts = _featurize_splif(protein_xyz, protein_ob, ligand_xyz, ligand_ob, self.contact_bins, pairwise_distances,
                  self.ecfp_degree)

    if "hbond" in self.voxel_feature_types:
      hbond_list = _compute_hydrogen_bonds(protein_xyz, protein_ob, ligand_xyz, ligand_ob, pairwise_distances,
                      self.hbond_dist_bins, self.hbond_angle_cutoffs, self.ecfp_degree)          

    if "sybyl" in self.voxel_feature_types:
      protein_sybyl_dict, ligand_sybyl_dict = _featurize_binding_pocket_sybyl(protein_xyz, protein_ob, ligand_xyz,
                                                                           ligand_ob, pairwise_distances, cutoff=7.0)

    if "pi_stack" in self.voxel_feature_types:
      protein_pi_t, protein_pi_parallel, ligand_pi_t, ligand_pi_parallel = _compute_pi_stack(protein_xyz, protein_ob,
                                                                                            ligand_xyz, ligand_ob, 
                                                                                            pairwise_distances)

    if "cation_pi" in self.voxel_feature_types:
      protein_cation_pi, ligand_cation_pi = _compute_binding_pocket_cation_pi(protein_xyz, protein_ob, ligand_xyz, ligand_ob)

    if "salt_bridge" in self.voxel_feature_types:
      salt_bridge_list = _compute_salt_bridges(protein_xyz, protein_ob, ligand_xyz, ligand_ob, pairwise_distances)
    ...
    if "voxel_combined" in self.feature_types:
      ...
      for system_id, system in transformed_systems.iteritems():
        ...
        feature_tensors = []
        if "ecfp" in self.voxel_feature_types:
          ecfp_tensor = self._voxelize(_convert_atom_to_voxel, _hash_ecfp, protein_xyz,
        ...
        if "sybyl" in self.voxel_feature_types:
          ...
          sybyl_tensor = self._voxelize(_convert_atom_to_voxel, hash_sybyl, protein_xyz, feature_dict=protein_sybyl_dict, 
        ...
        if "pi_stack" in self.voxel_feature_types:
          ...
          pi_parallel_tensor += self._voxelize(_convert_atom_to_voxel, None, ligand_xyz,
                                               feature_dict=ligand_pi_parallel, nb_channel=1)
          feature_tensors.append(pi_parallel_tensor)

          pi_t_tensor = self._voxelize(_convert_atom_to_voxel, None, protein_xyz,
        ...
        if "charge" in self.voxel_feature_types:
          ...
          charge_tensor += self._voxelize(_convert_atom_to_voxel, None, ligand_xyz,
  ...
  def _compute_flat_features(self, protein_xyz, protein_ob, ligand_xyz, ligand_ob):
    pairwise_distances = _compute_pairwise_distances(
      protein_xyz, ligand_xyz)
    protein_ecfp_dict, ligand_ecfp_dict = _featurize_binding_pocket_ecfp(protein_xyz, protein_ob, ligand_xyz,
                                      ligand_ob, pairwise_distances, cutoff=4.5, ecfp_degree=self.ecfp_degree)
    ...
    hbond_vectors = [self._vectorize(_hash_ecfp_pair, feature_list=hbond_list, channel_power=0)
