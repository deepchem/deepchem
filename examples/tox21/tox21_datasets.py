...
def load_tox21(featurizer='ECFP', split='index'):
  ...
  train, valid, test = os.path.join(dataset_dir, 'train'), os.path.join(dataset_dir, 'valid'), os.path.join(dataset_dir,
                                                                                                            'test')
  if os.path.isdir(dataset_dir):
    train, valid, test = DiskDataset(data_dir=train), DiskDataset(data_dir=valid), DiskDataset(data_dir=test)
    transformers = [
  ...
  train, valid, test = splitter.train_valid_test_split(dataset, train_dir=train, valid_dir=valid, test_dir=test)
