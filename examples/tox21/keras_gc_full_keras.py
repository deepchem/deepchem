...
class GraphGather(tf.keras.layers.Layer):
  ...
  def call(self, x, **kwargs):
    ...
    # Extract graph topology
    membership = tf.squeeze(x[1], axis=1)
...
atom_features = tf.keras.Input(dtype=tf.float32, shape=(75,))
degree_slice = tf.keras.Input(dtype=tf.int32, shape=(2,))
membership = tf.keras.Input(dtype=tf.int32, shape=(1,))

deg_adjs = []
for i in range(10):
  deg_adj = tf.keras.Input(dtype=tf.int32, shape=(i + 1,))
