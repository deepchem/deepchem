...
def retrieve_datasets():
  os.system('wget %s' % 'https://s3-us-west-1.amazonaws.com/deep-crystal-california/az_logd.csv')
  os.system('wget %s' % 'https://s3-us-west-1.amazonaws.com/deep-crystal-california/az_hppb.csv')
  os.system('wget %s' % 'https://s3-us-west-1.amazonaws.com/deep-crystal-california/az_clearance.csv')    
...
def experiment(dataset_file, method='GraphConv', split='scaffold'):
  ...
  if method == 'GraphConv':
    ...
    model = dc.models.TensorflowMultiTaskRegressor(
      len(tasks), train.get_data_shape()[0],
      layer_sizes=[384, 196], dropouts=[.25, .25],
      weight_init_stddevs=[.02, .02],
      bias_init_consts=[.1, .1], learning_rate=LR,
      penalty=LMBDA, penalty_type="l2", optimizer="adam", batch_size=BATCH_SIZE,
...
model, train, val, test, transformers = experiment(DATASET,
                                                   method=MODEL,
                                                   split=SPLIT)  
if MODEL == 'GraphConv':
  best_val_score = 0.0
  ...
  for l in xrange(0, MAX_EPOCH):
    ...
    latest_train_score = model.evaluate(train, [metric], transformers)['mean-pearson_r2_score']
    latest_val_score = model.evaluate(val, [metric], transformers)['mean-pearson_r2_score']
  ...
  train_score = model.evaluate(train, [metric], transformers)['mean-pearson_r2_score']
  val_score = model.evaluate(val, [metric], transformers)['mean-pearson_r2_score']
