...
def load_toxcast(base_dir, reload=True, frac_train=.8):
  """Load toxcast datasets. Does not do train/test split"""
  ...
  loader = DataLoader(tasks=TOXCAST_tasks,
                      smiles_field="smiles",
  ...
  TOXCAST_tasks = TOXCAST_tasks[:num_tasks]
  ...
  train_dataset = DiskDataset.from_numpy(train_dir, X_train, y_train,
                                     w_train, ids_train, TOXCAST_tasks)
  valid_dataset = DiskDataset.from_numpy(valid_dir, X_valid, y_valid,
                                     w_valid, ids_valid, TOXCAST_tasks)
  
  return TOXCAST_tasks, (train_dataset, valid_dataset), transformers
