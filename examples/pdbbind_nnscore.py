"""
Hyperparameter search script for NNScore on pdbbind data.
"""
...
def main():
  ...
  feature_files = ["/scratch/users/enf/deep-docking/shallow/pdbbind_core_vanilla_mlp_nnscore-mp/features/pdbbind_core_df.pkl.joblib"]

  tasks = ["label"]
  results_file = "nnscore_hyperparameter_results.csv"

  train_dataset, test_dataset = featurize_and_split(
      input_file, feature_dir, samples_dir, train_dir,
      test_dir, splittype, feature_types, input_transforms,
      output_transforms, tasks, feature_files=feature_files)
