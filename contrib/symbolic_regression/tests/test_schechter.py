...
>>>>>>> add test cases on deepchem data
    with open(path, newline="") as f:
        ...
        for row in reader:
<<<<<<< HEAD
            x_rows.append([float(row['L'])])
            y_vals.append(float(row['phi']))
=======
            x_rows.append([float(row["L"])])
            y_vals.append(float(row["phi"]))
>>>>>>> add test cases on deepchem data
...
def main():
>>>>>>> add test cases on deepchem data
    csv_path = os.path.join(os.path.dirname(__file__), "schechter.csv")
    ...
    plt.title("Schechter Function Data: equation_search vs PySR")
    plt.xlabel("L")
    plt.ylabel("phi")
    ...
    plt.savefig(os.path.join(REPO_ROOT, "outputs", "schechter_compare.png"), dpi=150)
    plt.show()
