...
>>>>>>> add test cases on deepchem data
    with open(path, newline="") as f:
        ...
        for row in reader:
<<<<<<< HEAD
            x_rows.append([float(row['P'])])
            y_vals.append(float(row['M']))
=======
            x_rows.append([float(row["P"])])
            y_vals.append(float(row["M"]))
>>>>>>> add test cases on deepchem data
...
def main():
>>>>>>> add test cases on deepchem data
    csv_path = os.path.join(os.path.dirname(__file__), "leavitt.csv")
    ...
    plt.title("Leavitt Law Data: equation_search vs PySR")
    plt.xlabel("P")
    plt.ylabel("M")
    ...
    plt.savefig(os.path.join(REPO_ROOT, "outputs", "leavitt_compare.png"), dpi=150)
    plt.show()
