...
>>>>>>> add test cases on deepchem data
    ...
    X = torch.tensor(a_vals, dtype=torch.float32).unsqueeze(1)
    y = torch.tensor(t_vals, dtype=torch.float32)
    return X, y
...
>>>>>>> add test cases on deepchem data
    ...
    plt.title("Kepler Data: equation_search vs PySR")
    ...
    plt.savefig(os.path.join(REPO_ROOT, "outputs", "kepler_compare.png"), dpi=150)
    plt.show()
