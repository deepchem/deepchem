...
>>>>>>> add test cases on deepchem data
    with open(path, newline="") as f:
        ...
        for row in reader:
<<<<<<< HEAD
            x_rows.append([float(row['m1']), float(row['m2']), float(row['r'])])
            y_vals.append(float(row['F']))
=======
            x_rows.append([float(row["m1"]), float(row["m2"]), float(row["r"])])
            y_vals.append(float(row["F"]))
>>>>>>> add test cases on deepchem data
...
def main():
>>>>>>> add test cases on deepchem data
    csv_path = os.path.join(os.path.dirname(__file__), "newton.csv")
    ...
    plt.title("Newton Data: equation_search vs PySR")
    plt.xlabel("m1")
    plt.ylabel("F")
    ...
    plt.savefig(os.path.join(REPO_ROOT, "outputs", "newton_compare.png"), dpi=150)
    plt.show()
