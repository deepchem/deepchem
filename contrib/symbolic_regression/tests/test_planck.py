...
>>>>>>> add test cases on deepchem data
    with open(path, newline="") as f:
        ...
        for row in reader:
<<<<<<< HEAD
            x_rows.append([float(row['lambda']), float(row['T'])])
            y_vals.append(float(row['B']))
=======
            x_rows.append([float(row["lambda"]), float(row["T"])])
            y_vals.append(float(row["B"]))
>>>>>>> add test cases on deepchem data
...
def main():
>>>>>>> add test cases on deepchem data
    csv_path = os.path.join(os.path.dirname(__file__), "planck.csv")
    ...
    plt.title("Planck Data: equation_search vs PySR")
    plt.xlabel("lambda")
    plt.ylabel("B")
    ...
    plt.savefig(os.path.join(REPO_ROOT, "outputs", "planck_compare.png"), dpi=150)
    plt.show()
