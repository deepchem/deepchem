...
class Tree:
    ...
    def forward(self, X):
        ...
        if spec is not None:
            if spec.arity != len(self.children):
                raise ValueError(
                    f"Arity mismatch for op '{op}': expected {spec.arity}, got {len(self.children)}"
                )
    ...
    def _sample_any_op(ops, rng=None):
        ...
        if not candidates:
            raise ValueError(
                "options.ops must contain at least one unary/binary operator"
            )
        return rng.choice(candidates)
    ...
    def mutate_constant(tree, temperature, options, rng=None):
        ...
        const_nodes = [
            n for n in tree.iter_nodes() if n.is_leaf() and n.is_constant_leaf()
        ]
        if not const_nodes:
            return tree
    ...
    def rotate_randomly(self, rng=None):
        ...
        pivot_idx = rng.choice(
            [i for i, ch in enumerate(root.children) if ch.degree > 0]
        )
