...
def run():
    ...
    try:
        ...
        local_ip = (
            "localhost:" + TF_CONFIG["cluster"][task_type][task_index].split(":")[1]
        )
        TF_CONFIG["cluster"][task_type][task_index] = local_ip
        if (task_type in ("chief", "master")) or (
    ...
    model = dc.models.MultitaskClassifier(
        n_tasks, n_features, layer_sizes=[1000], dropout=0.25
    )
    ...
    train_spec = tf.estimator.TrainSpec(
        input_fn=lambda: input_fn(train_dataset, 100), max_steps=100000
    )
    eval_spec = tf.estimator.EvalSpec(
        input_fn=lambda: input_fn(test_dataset, 1),
        steps=None,
        start_delay_secs=0,
        throttle_secs=30,
    )
